...
#		path => "['usr/bin','/usr/sbin']",
	...
	#extract tomcat
	exec {"/usr/bin/tar -xvf /var/tmp/apache-tomcat-9.0.21.tar.gz":
#		path => "['usr/bin','/usr/sbin']",
	...
	file {"/usr/local/tomcat9":
		...
		mode => "777",	
		before => Exec['/usr/local/tomcat9/apache-tomcat-9.0.21/bin/startup.sh'],
	}
	
	#move tomcat folder
	exec {"/usr/bin/mv -f /usr/tmp/nirav/apache-tomcat-9.0.21 /usr/local/tomcat9":
		path => "['usr/bin','/usr/sbin']",
		require => File['/usr/local/tomcat9'],
	}
	
	#set CATALINE_Home
	exec {'/bin/echo "export CATALINA_HOME=/usr/local/tomcat9/apache-tomcat-9.0.21" >> /root/.bashrc; source /root/.bashrc':
#		path => "/usr/bin/echo",
		require => Exec['/usr/bin/mv -f /usr/tmp/nirav/apache-tomcat-9.0.21 /usr/local/tomcat9'],	
	}

	#start service
	exec {"/usr/local/tomcat9/apache-tomcat-9.0.21/bin/startup.sh":
